...
abstract class Auth_Group_Driver extends \Auth_Driver {
	...
	public static function forge(array $config = array()) {
		...
		foreach ( $driver->get_config ( 'drivers', array () ) as $type => $drivers ) {
			foreach ( $drivers as $d => $custom ) {
				$custom = is_int ( $d ) ? array (
						'driver' => $custom 
				) : array_merge ( $custom, array (
						'driver' => $d 
				) );
	 ...
	 *
	 * @param
	 *        	mixed	condition to validate
	 * @param
	 *        	string	acl driver id or null to check all
	 * @param
	 *        	array	user identifier to check in form array(driver_id, user_id)
	 * @return bool
	 */
	 ...
	 *
	 * @param
	 *        	mixed	condition to check for access
	 * @param
	 *        	array	user identifier in the form of array(driver_id, user_id), or null for logged in
	 * @return bool
	 */
	 ...
	 *
	 * @param
	 *        	mixed	group condition to check
	 * @return string
	 */
