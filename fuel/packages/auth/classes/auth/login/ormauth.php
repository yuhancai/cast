...
class Auth_Login_Ormauth extends \Auth_Login_Driver {
	...
	public static function _init() {
		...
		if ($columns = \Config::get ( 'ormauth.table_columns', array () ) == array (
				'*' 
		) or ! is_array ( $columns )) {
	...
	public function validate_user($username_or_email = '', $password = '') {
		...
		$user = \Model\Auth_User::query ()->select ( \Config::get ( 'ormauth.table_columns', array () ) )->related ( 'metadata' )->where_open ()->where ( 'username', '=', $username_or_email )->or_where ( 'email', '=', $username_or_email )->where_close ()->where ( 'password', '=', $password )->get_one ();
	...
	public function force_login($user_id = '') {
		...
		if (! $user_id instanceof \Model\Auth_User) {
			$this->user = \Model\Auth_User::query ()->select ( \Config::get ( 'ormauth.table_columns', array () ) )->related ( 'metadata' )->where ( 'id', '=', $user_id )->get_one ();
		} else {
			$this->user = $user_id;
		}
	...
	public function logout() {
		...
		if (\Config::get ( 'ormauth.guest_login', true )) {
			$this->user = \Model\Auth_User::query ()->where ( 'id', '=', 0 )->get_one ();
		} else {
			$this->user = false;
		}
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array()) {
		...
		foreach ( $profile_fields as $field => $value ) {
			$user->metadata [] = \Model\Auth_Metadata::forge ( array (
					'key' => $field,
					'value' => $value 
			) );
		}
	...
	public function update_user($values, $username = null) {
		...
		$current_values = \Model\Auth_User::query ()->select ( \Config::get ( 'ormauth.table_columns', array () ) )->where ( 'username', '=', $username )->get_one ();
		...
		if (array_key_exists ( 'password', $values )) {
			if (empty ( $values ['old_password'] ) or $current_values->password != $this->hash_password ( trim ( $values ['old_password'] ) )) {
		...
		if (array_key_exists ( 'email', $values )) {
			...
			$matches = \Model\Auth_User::query ()->select ( \Config::get ( 'ormauth.table_columns', array () ) )->where ( 'email', '=', $email )->where ( 'id', '!=', $current_values->id )->get_one ();
	...
	public function reset_password($username) {
		// get the user object
		$user = \Model\Auth_User::query ()->select ( \Config::get ( 'ormauth.table_columns', array () ) )->where ( 'username', '=', $username )->get_one ();
	...
	public function delete_user($username) {
		...
		// get the user object
		$user = \Model\Auth_User::query ()->related ( 'metadata' )->select ( \Config::get ( 'ormauth.table_columns', array () ) )->where ( 'username', '=', $username )->get_one ();
