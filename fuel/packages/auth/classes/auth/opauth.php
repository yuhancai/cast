...
class Auth_Opauth {
	 ...
	 *
	 * @param
	 *        	array any call-time configuration to be used
	 * @param
	 *        	bool whether or not Opauth should run automatically
	 */
	...
	/**
	 * New Opauth login.
	 * If we know this user, we can perform a login, if
	...
	protected function callback() {
		...
		if ($this->get ( 'auth' ) === null or $this->get ( 'timestamp' ) === null or $this->get ( 'signature' ) === null or $this->get ( 'auth.provider' ) === null or $this->get ( 'auth.uid' ) === null) {
	 ...
	 *
	 * @param
	 *        	array array with the raw Opauth response user fields
	 *        	
	...
	protected function create_user(array $user) {
		$user_id = \Auth::create_user ( 
				...
				array (
						...
						'fullname' => isset ( $user ['name'] ) ? $user ['name'] : (isset ( $user ['full_name'] ) ? $user ['full_name'] : (isset ( $user ['first_name'], $user ['last_name'] ) ? $user ['first_name'] . ' ' . $user ['last_name'] : null)) 
				) );
