	...
	protected static function run_migration() {
		...
		foreach ( \Config::get ( 'simpleauth.roles', array () ) as $role => $config ) {
			...
			// no, add it
				...
				list ( $role_id, $rows_affected ) = \DB::insert ( static::$data ['ormauth_table'] . '_roles' )->set ( array (
						'name' => $role 
				) )->execute ();
			}
		...
		foreach ( \Config::get ( 'simpleauth.groups', array () ) as $group => $config ) {
			...
			// no, add it
				...
				list ( $group_id, $rows_affected ) = \DB::insert ( static::$data ['ormauth_table'] . '_groups' )->set ( array (
						'name' => $config ['name'] 
				) )->execute ();
			}
			
			// update the user group entries
			\DB::update ( static::$data ['ormauth_table'] )->set ( array (
					'group_id' => $group_id 
			) )->where ( 'group_id', '=', $group )->execute ();
			...
			if ($group) {
				foreach ( $config ['roles'] as $role ) {
					...
					if (! $role) {
						$role = \Model\Auth_Role::forge ( array (
								'name' => $role 
						) );
						\Cli::write ( '- creating role: ' . $role, 'light_green' );
					}
	...
	protected static function usertable() {
		if (! \DBUtil::table_exists ( static::$data ['ormauth_table'] )) {
			if (! \DBUtil::table_exists ( static::$data ['simpleauth_table'] )) {
				...
				\DBUtil::create_index ( static::$data ['ormauth_table'], array (
						'username',
						'email' 
				), 'username', 'UNIQUE' );
			} else {
