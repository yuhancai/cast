...
CONF;
	...
	public static function controller($args, $build = true) {
		...
		$class_name = \Inflector::classify ( str_replace ( array (
				'\\',
				'/' 
		), '_', $name ), false );
...
MODEL;
			...
			if (! \Cli::option ( 'no-timestamp' )) {
				...
				$timestamp_properties = array (
						$created_at . ':' . $time_type . ':null[1]',
						$updated_at . ':' . $time_type . ':null[1]' 
				);
				...
				if (\Cli::option ( 'soft-delete' )) {
					...
					$timestamp_properties = array_merge ( $timestamp_properties, array (
							$deleted_at . ':' . $time_type . ':null[1]' 
					) );
				}
...
MODEL;
	...
	public static function views($args, $subfolder, $build = true) {
		...
		$view_dir = APPPATH . 'views/' . trim ( str_replace ( array (
				'_',
				'-' 
		), DS, $controller ), DS ) . DS;
...
VIEW;
	...
	public static function migration($args, $build = true) {
		...
		$migration_name = \Str::lower ( str_replace ( array (
				'-',
				'/' 
		), '_', array_shift ( $args ) ) );
