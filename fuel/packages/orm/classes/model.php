...
class Model implements \ArrayAccess, \Iterator, \Sanitization {
	 ...
	 *
	 * @param
	 *        	bool	if true return the writeable connection (if set)
	 * @return null|string
	 */
	 ...
	 *
	 * @param
	 *        	string	type of condition to return
	 * @return array
	 */
	...
	public static function properties() {
		...
		if (empty ( $properties )) {
			try {
				...
				throw new \FuelException ( 'Listing columns failed, you have to set the model properties with a ' . 'static $_properties setting in the model. Original exception: ' . $e->getMessage () );
			}
		}
	 ...
	 *
	 * @param
	 *        	string specific observer to retrieve info of, allows direct param access by using dot notation
	 * @param
	 *        	mixed default return value when specific key wasn't found
	 * @return array
	 */
	 ...
	 *
	 * @param
	 *        	string	class name of the observer (including namespace)
	 * @param
	 *        	mixed	observer options
	 *        	
	 * @return void
	 */
	public static function register_observer($name, $options = null) {
		$class = get_called_class ();
		$new_observer = is_null ( $options ) ? array (
				$name 
		) : array (
				$name => $options 
		);
	...
	public static function find($id = null, array $options = array()) {
		...
		// Return specific request row by ID
			...
			if (array_key_exists ( get_called_class (), static::$_cached_objects ) and array_key_exists ( static::implode_pk ( $cache_pk ), static::$_cached_objects [get_called_class ()] ) and (! isset ( $options ['from_cache'] ) or $options ['from_cache'] == true)) {
	...
	public static function query($options = array()) {
		return Query::forge ( get_called_class (), array (
				static::connection (),
	...
	public function &get($property, array $conditions = array()) {
		...
		elseif ($rel = static::relations ( $property )) {
			if (! array_key_exists ( $property, $this->_data_relations )) {
				...
				$this->_update_original_relations ( array (
						$property 
				) );
			}
	...
	/**
	 * *
	 * Returns whether the given relation is fetched.
	 * If no relation is
	 *
	 ...
	 *
	 * @param
	 *        	Fieldset Fieldset instance to add fields to
	 * @param
	 *        	array|Model Model instance or array for use to repopulate
	 */
	 ...
	 *
	 * @param
	 *        	array assoc array with named values to store in the object
	 *        	
	 * @return Model this instance as a new object without primary key(s)
	 */
	 ...
	 * @param bool $isset,
	 *        	if true, do an exists check instead of returning the value
	 * @param bool $unset,
	 *        	if true, delete the EAV attribute if it exists
	 *        	
