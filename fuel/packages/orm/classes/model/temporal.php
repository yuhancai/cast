...
class Model_Temporal extends Model {
	...
	public static function query($options = array()) {
		...
		$query = Query_Temporal::forge ( get_called_class (), static::connection (), $options )->set_temporal_properties ( $max_timestamp, $timestamp_end_name, $timestamp_start_name );
	...
	public static function find_revisions_between($id, $earliestTime = null, $latestTime = null) {
		...
		$query = static::query ()->where ( 'id', $id )->where ( $timestamp_start_name, '>=', $earliestTime )->where ( $timestamp_start_name, '<=', $latestTime );
	 ...
	 * @param boolean $skip_temporal
	 *        	Skips temporal filtering on initial inserts. Should not be used!
	 * @return boolean
	 */
