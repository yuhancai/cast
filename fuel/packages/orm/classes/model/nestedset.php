...
class Model_Nestedset extends Model {
	...
	/*
	 * Initialize the nestedset model instance @param array any data passed to this model @param bool whether or not this is a new model instance @param string name of a database view to use instead of a table @param bool whether or not to cache this object @throws OutOfBoundsException if the model has a compound primary key defined
	 */
	 ...
	 *
	 * @param
	 *        	mixed type depends on the field type of the tree_field
	 ...
	 *
	 * @param
	 *        	bool whether or not to include related models
	 *        	
	 ...
	 *
	 * @param
	 *        	Model_Nestedset, or PK of the parent object, or null
	 * @return Model_Nestedset this object, for chaining
	 */
	 ...
	 *
	 * @param
	 *        	Model_Nestedset of the parent to check
	 * @return bool
	 */
	...
	public function is_descendant_of(Model_Nestedset $parent) {
		...
		return $this->{$left_field} > $parent->{$left_field} and $this->{$right_field} < $parent->{$right_field};
	}
	 ...
	 *
	 * @param
	 *        	Model_Nestedset of the child to check
	 * @return bool
	 */
	...
	public function is_ancestor_of(Model_Nestedset $child) {
		...
		return $child->{$left_field} > $this->{$left_field} and $child->{$right_field} < $this->{$right_field};
	}
	 ...
	 *
	 * @param
	 *        	Model_Nestedset object to verify against
	 * @return bool
	 */
	 ...
	 *
	 * @return mixed of false in case no valid object was found
	 */
	public function depth() {
		...
		if ($this->is_new ()) {
			...
			$query = $this->build_query ( false )->where ( $left_field, '<', $this->{$left_field} )->where ( $right_field, '>', $this->{$right_field} );
	 ...
	 *
	 * @param
	 *        	bool whether or not to return an array of objects
	 * @param
	 *        	string property name to store the node's children
	 * @return array
	 */
	...
	public function save($cascade = null, $use_transaction = false) {
		...
		if ($this->_is_new) {
			...
			// assume we want a new root node
				...
				$query = \DB::select ( $pk )->from ( static::table () )->where ( $left_field, '=', 1 );
	...
	/*
	 * Get a single tree node @param Query @returns mixed @throws BadMethodCallException if called without a parameter and without a node to fetch
	 */
	 ...
	 *        	Model_Nestedset
	 * @param
	 *        	string optional method name to display in the exception message
	 * @return bool
	 *
	...
	/*
	 * Fetch a node or nodes, and return the result @param string action, either 'single' or 'multiple' @return mixed Model_Nestedset or an array of Model_Nestedset, or null if none found @throws \UnexpectedValueException Relation was not found in the model
	 */
	protected function _fetch_nodes($action) {
		...
		switch ($this->_node_operation ['action']) {
			...
			case 'root' :
				$query = $this->build_query ()->where ( $left_field, '=', 1 );
				break;
			
			case 'roots' :
				$query = $this->query ()->where ( $left_field, '=', 1 );
				break;
			
			case 'parent' :
				$query = $this->build_query ()->where ( $left_field, '<', $this->{$left_field} )->where ( $right_field, '>', $this->{$right_field} )->order_by ( $right_field, 'ASC' );
				break;
			
			case 'first_child' :
				$query = $this->build_query ()->where ( $left_field, $this->{$left_field} + 1 );
				break;
			
			case 'last_child' :
				$query = $this->build_query ()->where ( $right_field, $this->{$right_field} - 1 );
				break;
			...
			case 'children' :
				...
				if (is_null ( $tree_field )) {
					$query = \DB::select ( 'child.' . $pk )->from ( array (
							static::table (),
							'child' 
					) )->join ( array (
							static::table (),
							'ancestor' 
					), 'left' )->on ( \DB::identifier ( 'ancestor.' . $left_field ), 'BETWEEN', \DB::expr ( ($left + 1) . ' AND ' . ($right - 1) ) )->on ( \DB::identifier ( 'child.' . $left_field ), 'BETWEEN', \DB::expr ( \DB::identifier ( 'ancestor.' . $left_field ) . ' + 1 AND ' . \DB::identifier ( 'ancestor.' . $right_field ) . ' - 1' ) )->where ( \DB::identifier ( 'child.' . $left_field ), 'BETWEEN', \DB::expr ( ($left + 1) . ' AND ' . ($right - 1) ) )->and_where ( 'ancestor.' . $pk, null );
				} else {
					$query = \DB::select ( 'child.' . $pk )->from ( array (
							static::table (),
							'child' 
							...
							static::table (),
							'ancestor' 
					), 'left' )->on ( \DB::identifier ( 'ancestor.' . $left_field ), 'BETWEEN', \DB::expr ( ($left + 1) . ' AND ' . ($right - 1) . ' AND ' . \DB::identifier ( 'ancestor.' . $tree_field ) . ' = ' . $this->get_tree_id () ) )->on ( \DB::identifier ( 'child.' . $left_field ), 'BETWEEN', \DB::expr ( \DB::identifier ( 'ancestor.' . $left_field ) . ' + 1 AND ' . \DB::identifier ( 'ancestor.' . $right_field ) . ' - 1' ) )->where ( \DB::identifier ( 'child.' . $left_field ), 'BETWEEN', \DB::expr ( ($left + 1) . ' AND ' . ($right - 1) ) )->and_where ( 'ancestor.' . $pk, null )->and_where ( 'child.' . $tree_field, '=', $this->get_tree_id () );
				}
				...
				$query = $this->build_query ()->where ( $pk, 'IN', $pks )->order_by ( $left_field, 'ASC' );
				break;
			...
			case 'descendants' :
				$query = $this->build_query ()->where ( $left_field, '>', $this->{$left_field} )->where ( $right_field, '<', $this->{$right_field} )->order_by ( $left_field, 'ASC' );
				break;
			
			case 'leaf_descendants' :
				$query = $this->build_query ()->where ( $left_field, '>', $this->{$left_field} )->where ( $right_field, '<', $this->{$right_field} )->where ( \DB::expr ( \DB::quote_identifier ( $right_field ) . ' - ' . \DB::quote_identifier ( $left_field ) ), '=', 1 )->order_by ( $left_field, 'ASC' );
				break;
			
			case 'previous_sibling' :
				$query = $this->build_query ()->where ( static::tree_config ( 'right_field' ), $this->{static::tree_config ( 'left_field' )} - 1 );
				break;
			
			case 'next_sibling' :
				$query = $this->build_query ()->where ( static::tree_config ( 'left_field' ), $this->{static::tree_config ( 'right_field' )} + 1 );
				break;
	 ...
	 *
	 * @param
	 *        	integer left pointer of the first node to shift
	 * @param
	 *        	integer number of positions to shift (if negative the shift will be to the left)
	 */
	 ...
	 *        	integer left pointer of the first node to shift
	 * @param
	 *        	integer right pointer of the last node to shift
	 * @param
	 *        	integer number of positions to shift (if negative the shift will be to the left)
	 */
	 ...
	 *
	 * @param
	 *        	integer new left pointer location to move to
	 */
