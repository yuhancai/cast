...
class Query_Temporal extends Query {
	 ...
	 * If set to null the latest revision
	 * is used.
	 *
	 * @param string $stamp
	 *        	Timestamp to look for
	 * @param string $timestamp_end_col
	 *        	Name of the end timestamp column
	 * @param string $timestamp_start_col
	...
	protected function modify_join_result($join_result, $name) {
		if (! is_null ( $this->timestamp ) and is_subclass_of ( $join_result [$name] ['model'], '\Orm\Model_Temporal' )) {
			...
			$join_result [$name] ['join_on'] [] = array (
					"$table.$this->timestamp_start_col",
					'<=',
					$query_time 
			);
			$join_result [$name] ['join_on'] [] = array (
					"$table.$this->timestamp_end_col",
					'>=',
					$query_time 
			);
		}
