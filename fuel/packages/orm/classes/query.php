...
class Query {
	 ...
	 *        	Optionally. Which field/fields must be retrieved
	 *        	
	 ...
	 *        	string Property
	 * @param
	 *        	string Comparison type (can be omitted)
	 * @param
	 ...
	 *
	 * @param
	 *        	\Fuel\Core\Database_Query_Builder_Where DB where() query object
	 * @param array $columns
	 *        	Optionally
	 * @param string $type
	 *        	Type of query to build (select/update/delete/insert)
	 *        	
	...
	public function build_query(\Fuel\Core\Database_Query_Builder_Where $query, $columns = array(), $type = 'select') {
		...
		foreach ( $models as $m ) {
			if (($type == 'select' and $m ['connection'] != $this->connection) or ($type != 'select' and $m ['connection'] != $this->write_connection)) {
	 ...
	 * @param array $result
	 *        	Current result array (by reference)
	 * @param string $model
	...
	public function count($column = null, $distinct = true) {
		...
		// Get the columns
		$columns = \DB::expr ( 'COUNT(' . ($distinct ? 'DISTINCT ' : '') . \Database_Connection::instance ( $this->connection )->quote_identifier ( $select ) . ') AS count_result' );
	...
	public function max($column) {
		...
		// Get the columns
		$columns = \DB::expr ( 'MAX(' . \Database_Connection::instance ( $this->connection )->quote_identifier ( $this->alias . '.' . $column ) . ') AS max_result' );
	...
	public function insert() {
		$res = \DB::insert ( call_user_func ( $this->model . '::table' ), array_keys ( $this->values ) )->values ( array_values ( $this->values ) )->execute ( $this->write_connection );
