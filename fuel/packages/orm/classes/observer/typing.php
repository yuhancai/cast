...
class Observer_Typing {
	 ...
	 * @param
	 *        	array any options to be passed
	 *        	
	 * @throws InvalidContentType
	 *
	 * @return string
	...
	public static function type_set_before($var, array $settings) {
		...
		foreach ( $values as $val ) {
			if (! in_array ( $val, $settings ['options'] )) {
				throw new InvalidContentType ( 'Invalid value given for ' . ucfirst ( $settings ['data_type'] ) . ', value "' . $var . '" not in available options: "' . implode ( ', ', $settings ['options'] ) . '".' );
			}
		}
