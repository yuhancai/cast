...
class Upload implements \ArrayAccess, \Iterator, \Countable {
	 ...
	 *        	Optional array of configuration items
	 *        	
	 ...
	 *
	 * @param int|string|array $selection
	 *        	Optional array index, element name or array with filter values
	 *        	
	 * @return void
	 */
	 ...
	 *        	Optional array index or element name
	 *        	
	 * @return File[]
	 */
	public function getAllFiles($index = null) {
		...
		is_array ( $selection ) or $selection = array (
				$selection 
		);
	...
	public function getValidFiles($index = null) {
		...
		if (is_numeric ( $index )) {
			...
			return isset ( $results [$index] ) ? array (
					$results [$index] 
			) : array ();
		} else {
			return $results;
	 ...
	 *        	Any valid callback, must accept a File object
	 *        	
	 ...
	 *        	name of the configuration item to set, or an array of configuration items
	 * @param mixed $value
	 *        	if $name is an item name, this holds the configuration values for that item
	 *        	
	 * @return void
	 */
	public function setConfig($item, $value = null) {
		// unify the parameters
		is_array ( $item ) or $item = array (
				$item => $value 
		);
	 ...
	 *        	Array of fieldnames to process, or null for all uploaded files
	 *        	
	 * @return void
	 */
		...
		foreach ( $_FILES as $name => $file ) {
			...
			if (is_array ( $file ['name'] )) {
				...
				if ($selection === null or in_array ( $name, $selection )) {
					$this->addFile ( array_merge ( array (
							'element' => $name,
							'filename' => null 
					), $file ) );
				}
	...
	protected function unifyFile($name, $file) {
		...
		foreach ( $file ['name'] as $key => $value ) {
			...
			if (is_array ( $value )) {
				...
				$data = array_merge ( $data, $this->unifyFile ( $name . '.' . $key, array (
						'filename' => null,
						'name' => $file ['name'] [$key],
						'type' => $file ['type'] [$key],
