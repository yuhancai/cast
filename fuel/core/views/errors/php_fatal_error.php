...
span.fuel_line_number {
	display: inline-block;
	background-color: #EFEFEF;
	padding: 4px 8px 4px 8px;
}

span.fuel_line_content {
	display: inline-block;
	padding: 4px 0 4px 4px;
}

span.fuel_current_line span.fuel_line_number, span.fuel_current_line {
...
<span
				<?php echo ($line_num == $error_line) ? ' class="fuel_line fuel_current_line"' : ' class="fuel_line"'; ?>><span
				class="fuel_line_number"><?php echo str_pad($line_num, (strlen(count($debug_lines))), ' ', STR_PAD_LEFT); ?></span><span
				class="fuel_line_content"><?php echo $line_content . PHP_EOL; ?>
...
<?php endif; ?>
		...
		foreach ( $backtrace as $trace ) :
			...
			?>
			<li><a href="#"
				onclick="javascript:fuel_toggle('backtrace_<?php echo $id; ?>');return false;"><?php echo \Fuel::clean_path($trace['file']).' @ line '.$trace['line']; ?></a>
...
<span
							<?php echo ($line_num == $trace['line']) ? ' class="fuel_line fuel_current_line"' : ' class="fuel_line"'; ?>><span
							class="fuel_line_number"><?php echo str_pad($line_num, (strlen(count($debug_lines))), ' ', STR_PAD_LEFT); ?></span><span
							class="fuel_line_content"><?php echo $line_content . PHP_EOL; ?>
</span></span><?php endforeach; ?></pre>
				</div></li>
...
<?php if (count($non_fatal) > 0): ?>
	...
	foreach ( $non_fatal as $err ) :
		...
		?>
			<li><a href="#"
				onclick="javascript:fuel_toggle('non_fatal_<?php echo $id; ?>');return false;"><?php echo $severity; ?>: <?php echo $message; ?> in <?php echo $filepath; ?> @ line <?php echo $error_line; ?></a>
...
<?php if ( ! empty($contents)): ?>
	<h2>
			Prior Contents (<a href="#"
				onclick="javascript:fuel_toggle('prior_contents');return false;">show</a>)
		</h2>
	<?php if (\Config::get('errors.render_prior', false) == true): ?>
		<div id="prior_contents" class="fuel_debug_source"
			style="display: none;"><?php echo $contents ?></div>
	<?php endif; ?>
	<?php if (\Config::get('errors.render_prior', false) == false): ?>
		<pre id="prior_contents" class="fuel_debug_source"
			style="display: none;"><?php echo e($contents) ?></pre>
	<?php endif; ?>
<?php endif; ?>
