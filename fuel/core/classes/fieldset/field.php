...
class Fieldset_Field {
	 ...
	 *
	 * @param
	 *        	Fieldset Fieldset to assign the field to
	 ...
	 *
	 * @param
	 *        	string|Callback	either a validation rule or full callback
	 * @return Fieldset_Field this, to allow chaining
	 */
	 ...
	 *        	string|Callback	either a validation rule or full callback
	 * @param
	 *        	bool	whether to also reset related attributes
	 * @return Fieldset_Field this, to allow chaining
	 */
	 ...
	 *        	string
	 * @param
	 *        	mixed new value or null to unset
	 * @return Fieldset_Field this, to allow chaining
	 */
	 ...
	 * @param
	 *        	mixed default output when attribute wasn't set
	 ...
	 *
	 * @param
	 *        	string|array one option value, or multiple value=>label pairs in an array
	 * @param
	 *        	string
	 * @param
	 *        	bool Whether or not to replace the current options
	 * @return Fieldset_Field this, to allow chaining
	 */
	...
	public function build() {
		...
		if ($form->get_config ( 'auto_id', false ) === true and $this->get_attribute ( 'id' ) == '') {
			$auto_id = $form->get_config ( 'auto_id_prefix', '' ) . str_replace ( array (
					'[',
					']' 
			), array (
					'-',
			...
			case 'checkbox' :
				if ($this->options) {
					...
					foreach ( $this->options as $value => $label ) {
						...
						$build_field [$form->label ( $label, null, array (
								'for' => $attributes ['id'] 
						) )] = $this->type == 'radio' ? $form->radio ( $attributes ) : $form->checkbox ( $attributes );
	...
	protected function template($build_field) {
		...
		$label = $this->label ? $form->label ( $this->label, null, array (
				'id' => 'label_' . $this->name,
