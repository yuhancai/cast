...
class Fieldset {
	 ...
	 *
	 * @param
	 *        	string Identifier for this fieldset
	 * @param
	 ...
	 *
	 * @param
	 *        	Fieldset parent fieldset to which this belongs
	 * @return Fieldset
	 */
	 ...
	 *
	 * @param
	 *        	string|null field name or null to fetch an array of all
	 * @param
	 *        	bool whether to get the fields array or flattened array
	 * @param
	 *        	bool whether to include tabular form fields in the flattened array
	 ...
	 *
	 * @param
	 *        	string|Object either a full classname (including full namespace) or object instance
	 * @param
	 *        	array|Object array or object that has the exactly same named properties to populate the fields
	 * @param
	 *        	string method name to call on model for field fetching
	 * @return Fieldset this, to allow chaining
	 */
	public function add_model($class, $instance = null, $method = 'set_form_fields') {
		...
		if ((is_string ( $class ) and is_callable ( $callback = array (
				'\\' . $class,
				$method 
		) )) || is_callable ( $callback = array (
				$class,
				$method 
		) )) {
	 ...
	 *
	 * @param
	 *        	string Model on which to define the tabular form
	 * @param
	 *        	string Relation of the Model on the tabular form is modeled
	 * @param
	 *        	array Collection of Model objects from a many relation
	 * @param
	 *        	int Number of empty rows to generate
	 *        	
	 * @return Fieldset this, to allow chaining
	 */
	public function set_tabular_form($model, $relation, $parent, $blanks = 1) {
		...
		foreach ( $parent->{$relation} as $row ) {
			...
			$fieldset->add ( $this->tabular_form_relation . '[' . $row->{$primary_key} . '][_delete]', '', array (
					'type' => 'checkbox',
					'value' => 1 
			) );
		}
		...
		for($i = 0; $i < $blanks; $i ++) {
			...
			$fieldset->add ( $this->tabular_form_relation . '_new[' . $i . '][_delete]', '', array (
					'type' => 'checkbox',
					'value' => 0,
					'disabled' => 'disabled' 
			) );
