			...
			if ($signature == $this->central_signature_end) {
				...
				while ( $signature == $this->dir_signature ) {
					...
					$this->central_dir_list [$dir ['file_name']] = array (
							...
							'crc-32' => str_pad ( dechex ( ord ( $dir ['crc-32'] [3] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $dir ['crc-32'] [2] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $dir ['crc-32'] [1] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $dir ['crc-32'] [0] ) ), 2, '0', STR_PAD_LEFT ),
	...
	private function _get_file_header(&$fh, $start_offset = false) {
		...
		if ($signature == $this->zip_signature) {
			...
			$i = array (
					...
					'lastmod_datetime' => mktime ( $last_mod_hour, $last_mod_minute, $last_mod_second, $last_mod_month, $last_mod_day, $last_mod_year ),
					'crc-32' => str_pad ( dechex ( ord ( $file ['crc-32'] [3] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $file ['crc-32'] [2] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $file ['crc-32'] [1] ) ), 2, '0', STR_PAD_LEFT ) . str_pad ( dechex ( ord ( $file ['crc-32'] [0] ) ), 2, '0', STR_PAD_LEFT ),
