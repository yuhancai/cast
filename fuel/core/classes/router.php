...
class Router {
	 ...
	 *        	string
	 * @param
	 *        	string|array|Route either the translation for $path, an array for verb routing or an instance of Route
	 * @param
	 *        	bool whether to prepend the route(s) to the routes array
	 */
			...
			if (preg_match_all ( '#\((?:\?P<(\w+?)>)?.*?\)#', $url, $matches ) !== false) {
				if (count ( $matches ) == 2) {
					...
					foreach ( $matches [0] as $index => $target ) {
						$replace = '';
						if (array_key_exists ( $key = $matches [1] [$index], $named_params ) || array_key_exists ( $key = '$' . ($index + 1), $named_params ) || array_key_exists ( $key = $indexed_group_count ++, $named_params )) {
	 ...
	 *
	 * @param
	 *        	Request		the given Request object
	 * @param
	 *        	bool		whether to use the defined routes or not
	 * @return mixed match array or false
	 */
