...
class Image_Gd extends \Image_Driver {
	...
	protected $accepted_extensions = array (
			'png',
			'gif',
			'jpg',
			'jpeg' 
	...
	protected function _watermark($filename, $position, $padding = 5) {
		...
		if ($values == false) {
			...
			if ($x < 0 || $y < 0) {
				...
				imagecopy ( $tmpwatermark, $watermark, 				// Copy the new image into the tmp watermark
				0, 0, $x < 0 ? abs ( $x ) : 0, $y < 0 ? abs ( $y ) : 0, $newwidth, $newheight );
	...
	protected function _flip($mode) {
		...
		$source = array_merge ( $sizes, array (
				'x' => 0,
				'y' => 0 
		) );
		...
		imagecopyresampled ( $image, $this->image_data, 0, 0, $source ['x'], $source ['y'], $sizes ['width'], $sizes ['height'], $source ['width'], $source ['height'] );
				...
				if ($dist <= $radius + $antialias) {
					...
					if ($fromCirc > 0) {
						if ($fromCirc == 0) {
							...
							$tmpColor = imagecolorallocatealpha ( $image, ($rgba >> 16) & 0xFF, 							// Red
							($rgba >> 8) & 0xFF, 							// Green
							$rgba & 0xFF, 							// Blue
