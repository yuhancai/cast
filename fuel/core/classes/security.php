...
class Security {
	...
	public static function _init() {
		...
		if (\Config::get ( 'security.csrf_autoload', true )) {
			$check_token_methods = \Config::get ( 'security.csrf_autoload_methods', array (
					'post',
					'put',
					'delete' 
			) );
	...
	public static function clean_uri($uri, $strict = false) {
		...
		$strict and $uri = preg_replace ( array (
				"/\.+\//",
				'/\/+/' 
		), '/', $uri );
	...
	public static function xss_clean($value, array $options = array()) {
		if (! is_array ( $value )) {
			...
			return htmLawed ( $value, array_merge ( array (
					'safe' => 1,
					'balanced' => 0 
			), $options ) );
		}
	 ...
	 *
	 * @param
	 *        	string CSRF token to be checked, checks post when empty
	 * @return bool
	 */
