...
class Theme {
	 ...
	 * If a config is given, it will not use the config
	 * file.
	 *
	 * @param array $config
	 *        	Optional config override
	...
	public function __construct(array $config = array()) {
		...
		$this->asset = \Asset::forge ( 'theme_' . spl_object_hash ( $this ), array (
				'paths' => array () 
		) );
	 ...
	 * This theme will be used if a view or asset
	 ...
	 * @param string|View|ViewModel $view
	 *        	or name of the view
	 * @param bool $overwrite
	 *        	true overwrite any already defined partials for this section
	 * @return View
	 */
	 ...
	 *
	 * @param string $section
	 *        	of the partial section in the template
	 * @param string $view
	 *        	of the view
	 * @return View
	 * @throws \ThemeException
	 ...
	 * @param string $var
	 *        	of the variable in the chome that will output the partial
	 *        	
	...
	public function load_info($theme = null) {
		...
		if (($file = $this->find_file ( $this->config ['info_file_name'], array (
				$theme 
		) )) == $this->config ['info_file_name']) {
	 ...
	 * If enabled, every theme view loaded
	...
	protected function find_file($view, $themes = null) {
		...
		foreach ( $themes as $theme ) {
			...
			$file = (pathinfo ( $view, PATHINFO_DIRNAME ) ? str_replace ( array (
					'/',
					DS 
			), DS, pathinfo ( $view, PATHINFO_DIRNAME ) ) . DS : '') . pathinfo ( $view, PATHINFO_FILENAME );
