...
class Request_Soap extends \Request_Driver {
	protected static $wsdl_settings = array (
			'wsdl',
			'classmap',
			'cache_wsdl' 
	);
	protected static $non_wsdl_settings = array (
			'location',
			'uri',
			'style',
			'use' 
	...
	/**
	 *
	 * @var \SoapClient the SoapClient object used for the connection
	 */
	...
	public function set_cookie($name, $value = null) {
		is_null ( $value ) ? $this->connection ()->__setCookie ( $name ) : $this->connection ()->__setCookie ( $name, $value );
	}
