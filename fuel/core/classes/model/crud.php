...
class Model_Crud extends \Model implements \Iterator, \ArrayAccess, \Serializable, \Sanitization {
	...
	public static function find($config = array(), $key = null) {
		$query = \DB::select ()->from ( static::$_table_name )->as_object ( get_called_class () );
		...
		if ($config instanceof \Closure) {
			...
			is_string ( $select ) and $select = array (
					$select 
			);
	 ...
	 *
	 * @param
	 *        	string Column to count by
	 * @param
	 *        	bool Whether to count only distinct rows (by column)
	 * @param
	 *        	array Query where clause(s)
	 * @param
	 *        	string Column to group by
