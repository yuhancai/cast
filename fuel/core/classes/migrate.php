...
class Migrate {
	/**
	 *
	 * @var array migrations registered in the database
	 */
	protected static $migrations = array ();
	
	/**
	 *
	 * @var string classes namespace prefix
	 */
	protected static $prefix = 'Fuel\\Migrations\\';
	
	/**
	 *
	 * @var string of the migration table
	 */
	 ...
	 *
	 * @param
	 *        	mixed	version to migrate to (up or down!)
	 * @param
	 *        	string name of the package, module or app
	 * @param
	 *        	string type of migration (package, module or app)
	 * @param
	 *        	bool	if true, also run out-of-sequence migrations
	 *        	
	 ...
	 *
	 * @param
	 *        	mixed	version to migrate up to
	 * @param
	 *        	string name of the package, module or app
	 * @param
	 *        	string type of migration (package, module or app)
	...
	public static function up($version = null, $name = 'default', $type = 'app') {
		...
		if (! empty ( $migrations )) {
			...
			is_null ( $version ) and $migrations = array (
					reset ( $migrations ) 
			);
	 ...
	 *
	 * @param
	 *        	mixed	version to migrate down to
	 * @param
	 *        	string name of the package, module or app
	 * @param
	 *        	string type of migration (package, module or app)
	 ...
	 *
	 * @param
	 *        	array	list of files to migrate
	 * @param
	 *        	string name of the package, module or app
	 * @param
	 *        	string type of migration (package, module or app)
	 * @param
	 *        	string method to call on the migration
	 *        	
	 * @return array
	 */
	 ...
	 *        	string type of migration (package, module or app)
	 * @param
	 *        	string name of the migration file just run
	 *        	
	 * @return void
	 */
	...
	protected static function write_revert($name, $type, $file) {
		...
		\DB::delete ( static::$table )->where ( 'name', $name )->where ( 'type', $type )->where ( 'migration', $file )->execute ( static::$connection );
	 ...
	 *        	string type of migration (package, module or app)
	 * @param
	 *        	mixed	version to start migrations from, or null to start at the beginning
	 * @param
	 *        	mixed	version to end migrations by, or null to migrate to the end
	 *        	
	 * @return array
	 */
		...
		foreach ( $files as $file ) {
			...
			if ((is_null ( $start ) or $migration > $start) and (is_null ( $end ) or $migration <= $end)) {
				...
				if (in_array ( basename ( $file, '.php' ), $current )) {
					...
					$direction == 'down' and $migrations [$migration] = array (
							'path' => $file 
					);
				} else {
					// not installed yet. store it only if we're going up
					$direction == 'up' and $migrations [$migration] = array (
							'path' => $file 
					);
				}
			}
	 ...
	 *
	 * @param
	 *        	string	name of the app (not used at the moment)
	 *        	
	 * @return array
	 */
	...
	protected static function _find_package($name = null) {
		...
		if ($name) {
			// find a package
			foreach ( \Config::get ( 'package_paths', array (
					PKGPATH 
			) ) as $p ) {
	...
	protected static function table_version_check() {
		...
		elseif (! \DBUtil::field_exists ( static::$table, array (
				'migration' 
		) )) {
