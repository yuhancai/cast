...
class DBUtil {
	 ...
	 *        	to add an IF NOT EXISTS statement.
	...
	protected static function alter_fields($type, $table, $fields, $db = null) {
		...
		if ($type === 'DROP') {
			...
			$use_brackets = ! in_array ( $type, array (
					'ADD',
					'CHANGE',
					'MODIFY' 
			) );
			...
			if (is_array ( $index_columns )) {
				foreach ( $index_columns as $key => $value ) {
					if (is_numeric ( $key )) {
						...
						$index_name .= ($index_name == '' ? '' : '_') . str_replace ( array (
								'(',
								')',
								' ' 
						), '', $key );
	 ...
	 * @param string $db
	 *        	the database name in the config
	...
	public static function add_foreign_key($table, $foreign_key) {
		...
		$sql .= 'ADD ';
		$sql .= ltrim ( static::process_foreign_keys ( array (
				$foreign_key 
		) ), ',' );
	...
	public static function truncate_table($table, $db = null) {
		return \DB::query ( 'TRUNCATE TABLE ' . \DB::quote_identifier ( \DB::table_prefix ( $table, $db ? $db : static::$connection ), $db ? $db : static::$connection ), \DB::DELETE )->execute ( $db ? $db : static::$connection );
	}
	...
	/*
	 * Executes table maintenance. Will throw FuelException when the operation is not supported. @throws	FuelException @param string $table the table name @return bool whether the operation has succeeded
	 */
	protected static function table_maintenance($operation, $table, $db = null) {
		...
		if ($type === 'status' and in_array ( strtolower ( $message ), array (
				'ok',
