...
class File_Area {
	...
	/**
	 *
	 * @var string to basedir restriction, null for no restriction
	 */
	protected $basedir = null;
	
	/**
	 *
	 * @var array of allowed extensions, null for all
	 */
	protected $extensions = null;
	
	/**
	 *
	 * @var string url for files, null for not available
	 */
	protected $url = null;
	
	/**
	 *
	 * @var bool or not to use file locks when doing file operations
	 */
	protected $use_locks = false;
	
	/**
	 *
	 * @var array file handler per file extension
	 */
	 ...
	 * @param
	 *        	array				optional config
	 * @return File_Handler_File
	 * @throws FileAccessException outside basedir restriction or disallowed file extension
	 */
	...
	public function get_path($path) {
		...
		if (! empty ( $this->basedir ) && substr ( $pathinfo ['dirname'], 0, strlen ( $this->basedir ) ) == $this->basedir) {
			...
			$pathinfo ['dirname'] = (! empty ( $this->basedir ) ? realpath ( $this->basedir . DS . $pathinfo ['dirname'] ) : realpath ( $pathinfo ['dirname'] )) ?  : (! empty ( $this->basedir ) ? $this->basedir . DS . str_replace ( '..', '', $pathinfo ['dirname'] ) : $pathinfo ['dirname']);
		}
	 ...
	 *        	string
	 * @return string
	 * @throws LogicException no url is set or no basedir is set and file is outside DOCROOT
	 */
