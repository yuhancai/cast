...
class Pagination {
	...
	public static function __callStatic($name, $arguments) {
		...
		if ($instance = static::instance () and method_exists ( $instance, $name )) {
			return call_fuel_func_array ( array (
					$instance,
					$name 
			), $arguments );
		}
	 ...
	 *
	 * @return \Pagination new pagination instance
	 */
	 ...
	 *
	 * @return \Pagination existing pagination instance
	 */
	 ...
	 *
	 * @return mixed Markup for page number links, or an array of raw pagination data
	 */
	public function render($raw = false) {
		...
		$html = str_replace ( '{pagination}', $this->first () . $this->previous () . $this->pages_render () . $this->next () . $this->last (), $this->template ['wrapper'] );
	 ...
	 *
	 * @return string for the pagination block
	 */
	public function pages_render() {
		...
		for($i = $start; $i <= $end; $i ++) {
			if ($this->config ['calculated_page'] == $i) {
				$html .= str_replace ( '{link}', str_replace ( array (
						'{uri}',
						'{page}' 
				), array (
						'#',
						$i 
				), $this->template ['active-link'] ), $this->template ['active'] );
				$this->raw_results [] = array (
						'uri' => '#',
						'title' => $i,
						'type' => 'active' 
				...
				$html .= str_replace ( '{link}', str_replace ( array (
						...
						$this->_make_link ( $i ),
						$i 
				), $this->template ['regular-link'] ), $this->template ['regular'] );
				$this->raw_results [] = array (
						'uri' => $this->_make_link ( $i ),
						'title' => $i,
						'type' => 'regular' 
				);
			}
	 ...
	 *        	
	 * @return string for the 'first' page number link
	 */
		...
		if ($this->config ['show_first']) {
			if ($this->config ['total_pages'] > 1 and $this->config ['calculated_page'] > 1) {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						$this->_make_link ( 1 ),
						$marker 
				), $this->template ['first-link'] ), $this->template ['first'] );
				$this->raw_results ['first'] = array (
						'uri' => $this->_make_link ( 1 ),
						'title' => $marker,
						'type' => 'first' 
				);
			} else {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						'#',
						$marker 
				), $this->template ['first-inactive-link'] ), $this->template ['first-inactive'] );
				$this->raw_results ['first'] = array (
						'uri' => '#',
						'title' => $marker,
						'type' => 'first-inactive' 
	 ...
	 *        	optional text to display in the link
	 *        	
	 * @return string for the 'previous' page number link
	 */
		...
		if ($this->config ['total_pages'] > 1) {
			if ($this->config ['calculated_page'] == 1) {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						'#',
						$marker 
				), $this->template ['previous-inactive-link'] ), $this->template ['previous-inactive'] );
				$this->raw_results ['previous'] = array (
						'uri' => '#',
						'title' => $marker,
						'type' => 'previous-inactive' 
				);
			} else {
				...
				$html = str_replace ( '{link}', str_replace ( array (
						...
						$this->_make_link ( $previous_page ),
						$marker 
				), $this->template ['previous-link'] ), $this->template ['previous'] );
				$this->raw_results ['previous'] = array (
						'uri' => $this->_make_link ( $previous_page ),
						'title' => $marker,
						'type' => 'previous' 
				);
			}
	 ...
	 *        	optional text to display in the link
	 *        	
	 * @return string for the 'next' page number link
	 */
		...
		if ($this->config ['total_pages'] > 1) {
			if ($this->config ['calculated_page'] == $this->config ['total_pages']) {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						'#',
						$marker 
				), $this->template ['next-inactive-link'] ), $this->template ['next-inactive'] );
				$this->raw_results ['next'] = array (
						'uri' => '#',
						'title' => $marker,
						'type' => 'next-inactive' 
				);
				...
				$html = str_replace ( '{link}', str_replace ( array (
						...
						$this->_make_link ( $next_page ),
						$marker 
				), $this->template ['next-link'] ), $this->template ['next'] );
				$this->raw_results ['next'] = array (
						'uri' => $this->_make_link ( $next_page ),
						'title' => $marker,
						'type' => 'next' 
				);
			}
	 ...
	 *        	optional text to display in the link
	 *        	
	 * @return string for the 'last' page number link
	 */
		...
		if ($this->config ['show_last']) {
			if ($this->config ['total_pages'] > 1 and $this->config ['calculated_page'] != $this->config ['total_pages']) {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						$this->_make_link ( $this->config ['total_pages'] ),
						$marker 
				), $this->template ['last-link'] ), $this->template ['last'] );
				$this->raw_results ['last'] = array (
						'uri' => $this->_make_link ( $this->config ['total_pages'] ),
						'title' => $marker,
						'type' => 'last' 
				);
			} else {
				$html = str_replace ( '{link}', str_replace ( array (
						...
						'#',
						$marker 
				), $this->template ['last-inactive-link'] ), $this->template ['last-inactive'] );
				$this->raw_results ['last'] = array (
						'uri' => '#',
						'title' => $marker,
						'type' => 'last-inactive' 
