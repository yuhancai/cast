...
class Form_Instance {
	...
	public function __construct($fieldset, array $config = array()) {
		if ($fieldset instanceof Fieldset) {
			...
			$this->fieldset = \Fieldset::forge ( $fieldset, array (
					'form_instance' => $this 
			) );
		}
	 ...
	 *
	 * @param
	 *        	string|array action string or array with more tag attribute settings
	 * @return string
	 */
	public function open($attributes = array(), array $hidden = array()) {
		$attributes = ! is_array ( $attributes ) ? array (
				'action' => $attributes 
		) : $attributes;
	 ...
	 *
	 * @param
	 *        	array array with tag attribute settings
	 * @param
	 *        	string string for the fieldset legend
	 * @return string
	 */
	 ...
	 *
	 * @param
	 *        	string|array either fieldname or full attributes array (when array other params are ignored)
	 * @param
	 *        	string
	 * @param
	 *        	array
	 ...
	 *        	string
	 * @param
	 *        	mixed either attributes (array) or bool/string to set checked status
	 * @param
	 *        	array
	 * @return string
	 */
	 ...
	 *        	string|array either fieldname or full attributes array (when array other params are ignored)
	 * @param
	 *        	string selected value(s)
	 * @param
	 *        	array array of options and option groups
	 * @param
	 *        	array
	 * @return string
	 */
		...
		$listoptions = function (array $options, $selected, $level = 1) use(&$listoptions, &$current_obj, &$attributes) {
			...
			foreach ( $options as $key => $val ) {
				if (is_array ( $val )) {
					...
					$input .= str_repeat ( "\t", $level ) . html_tag ( 'optgroup', array (
							'label' => $key,
							'style' => 'text-indent: ' . (20 + 10 * ($level - 1)) . 'px;' 
					), $optgroup ) . PHP_EOL;
				} else {
					$opt_attr = array (
							'value' => $key 
					...
					$opt_attr ['value'] = ($current_obj->get_config ( 'prep_value', true ) && empty ( $attributes ['dont_prep'] )) ? $current_obj->prep_value ( $opt_attr ['value'] ) : $opt_attr ['value'];
					$val = ($current_obj->get_config ( 'prep_value', true ) && empty ( $attributes ['dont_prep'] )) ? $current_obj->prep_value ( $val ) : $val;
	 ...
	 *
	 * @param
	 *        	string|Fieldset_Field field instance or name of a field in this form's fieldset
	 * @return string @depricated until v1.2
	 */
	...
	public function add_csrf() {
		$this->add ( \Config::get ( 'security.csrf_token_key', 'fuel_csrf_token' ), 'CSRF Token' )->set_type ( 'hidden' )->set_value ( \Security::fetch_token () )->add_rule ( array (
				'Security',
				'check_token' 
		) );
	 ...
	 *
	 * @param
	 *        	string|array a single key or multiple in an array, empty to fetch all
	 * @param
	 *        	mixed default output when config wasn't set
	 ...
	 *
	 * @return Validation to allow chaining
	 */
