...
class Cache_Storage_Apc extends \Cache_Storage_Driver {
	...
	public function delete_all($section) {
		...
		if (is_array ( $index )) {
			...
			if (! empty ( $section )) {
				$dirs = in_array ( $section, $index ) ? array (
						$section 
				) : array ();
			} else {
				$dirs = $index;
	 ...
	 *
	 * @param
	 *        	string name of the config variable to validate
	 * @param
	 *        	mixed value
	 * @return mixed
	 */
	 ...
	 *
	 * @param
	 *        	bool if true, remove the key retrieved from the index
	 * @return string
	 */
	...
	protected function _get_index() {
		...
		return array (
				$identifier,
				$sections,
				apc_fetch ( $this->config ['cache_id'] . $sections ) 
		);
	}
	...
	protected function _update_index($key) {
		...
		$index [$identifier] = array (
				$key,
				$this->created 
		);
		apc_store ( $this->config ['cache_id'] . $sections, array_merge ( $index, array (
				$identifier => array (
						$key,
						$this->created 
				) 
